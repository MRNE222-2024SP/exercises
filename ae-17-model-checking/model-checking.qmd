...
Plot bill length vs bill depth and add a linear fit line using `geom_smooth(method = "lm")`. Add a linear fit line using `geom_smooth(method = "lm")`.
```{r}
...
Using the `bill_aug` dataframe, plot the `.fitted` values vs. the `.resid` values. Are there any discernible patterns and or trends in the fitted vs. residuals plot?
```{r}
