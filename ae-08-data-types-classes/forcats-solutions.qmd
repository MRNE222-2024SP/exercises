...
```{r}
customers %>%
  mutate(customer_type = fct_shuffle(customer_type)) %>%
...
```{r}
customers %>%
  mutate(customer_type = fct_rev(customer_type)) %>%
  ggplot(aes(x = customer_type, y = adr)) +
    geom_boxplot()
```
...
```{r}
customers %>%
  mutate(customer_type = fct_shift(customer_type)) %>%
  ggplot(aes(x = customer_type, y = adr)) +
    geom_boxplot()
```
...
```{r}
customers %>%
  mutate(customer_type = fct_relevel(customer_type, c("Transient-Party", "Group", "Transient", "Contract"))) %>%
  ggplot(aes(x = customer_type, y = adr)) +
    geom_boxplot()
```
...
```{r}
customers %>%
  mutate(customer_type = fct_recode(customer_type, `Party barge` = "Transient-Party", Peeps = "Group", `Free spirit` = "Transient", Regular = "Contract")) %>%
  ggplot(aes(x = customer_type, y = adr)) +
    geom_boxplot()
```
